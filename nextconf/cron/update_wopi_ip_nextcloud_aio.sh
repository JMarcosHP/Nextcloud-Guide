#!/bin/bash
# Script to automatically update Nextcloud-AIO collabora code WOPI IP of your public domain. Requires the "richdocuments" app installed on your server, needs to run as root user or a user added in the docker group.
# To set an automated cronjob:
# sudo crontab -e
...
current_wopi_list=$(docker exec -u www-data nextcloud-aio-nextcloud bash -c "php occ config:app:get richdocuments wopi_allowlist")
...
if [ $update_needed -eq 1 ]; then
    ...
    docker exec -u www-data nextcloud-aio-nextcloud bash -c "php occ config:app:set richdocuments wopi_allowlist --value=\"$new_ip_list\""
